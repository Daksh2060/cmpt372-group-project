...
async function getStops(route: string, direction: string, date: string) {
  ...
  const res = await fetch(
    `${SERVER}/routes/${route}/stops?direction=${direction}&date=${
      date.split("T")[0]
    }`
  );
  if (!res.ok) {
...
export default function StaticTimetable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        //Adjust  this
        const res = await fetch(`http://127.0.0.1:8080/routes`);
        console.log(res);
        if (!res.ok) {
          throw new Error(`Server responded with status ${res.status}`);
        }
        const data = await res.json(); // Parsing JSON data
        console.log(data);
        setRoutes(data); // Setting the parsed JSON data to state
      } catch (error) {
      ...
      <div id={"options"}>
          ...
          <select onChange={(event) => setSelectedRoute(event.target.value)}>
            ...
            {routes.map((value) => (
              <option
                key={value.name}
                value={value.name}
              >{`${value.name} ${value.destinations}`}</option>
            ))}
          </select>
          <button
            onClick={() => {
        ...
        <div>
          ...
          >
            ...
            {directions.map((value) => (
              <option
                key={`${value.direction_id}${value.trip_headsign}`}
        ...
        <div>
          <select onChange={(event) => setStartStop(event.target.value)}>
            ...
            {stops.map((value) => (
              <option key={value.stop_id} value={value.stop_code}>
                {value.stop_name}
              </option>
            ))}
          </select>
          ...
          <button
            onClick={() => {
              ...
              if (times.length > 0) {
                prevTimes = times[times.length - 1].times.map(
                  (value) => value.endTime
                );
              } else {
                ...
                .then((value) => {
                  setTimes(
                    times.concat([
                      {
                        BULL: Date.now(),
                        startStopName: value.startStopName,
                        endStopName: value.endStopName,
            ...
            {times.map((value, index) => (
              <div className={"route"} key={value.BULL}>
                ...
                <button
                  ...
                  onClick={() =>
                    setTimes(
                      times.filter((value2) => value.BULL !== value2.BULL)
                    )
                  }
                >
                  Remove
